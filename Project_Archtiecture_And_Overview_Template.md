# 🏥 {{PROJECT_NAME}} Development & Architecture Guide
...
| **`{{PROJECT_NAME}}.WebApp`** | {{FRONTEND_FRAMEWORK}} | Main web client application. |
| **`{{PROJECT_NAME}}.Mobile`** | {{MOBILE_FRAMEWORK}} | Cross-platform mobile client application. |
...
4. **JWT Auth**: Protect endpoints using `[Authorize(Roles = "...")]`.
5. **Real-time**: Hook events to real-time hubs (e.g., SignalR) for push notifications.
